    ...
    <div className="animate-pulse bg-white/70 backdrop-blur-xl rounded-2xl border border-white/20 shadow-xl p-6">
      <div className="flex items-center justify-between">
        <div className="space-y-2">
          <div className="h-4 bg-brand-slate-200 rounded w-32" />
          <div className="h-3 bg-brand-slate-100 rounded w-48" />
        </div>
        <div className="h-6 bg-brand-slate-100 rounded-full w-24" />
      </div>
    </div>
  );
}
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative w-full max-w-2xl bg-white/90 backdrop-blur-xl rounded-2xl shadow-2xl border border-white/20 overflow-hidden animate-slide-up">
        ...
        {/* Header */}
        <div className="flex items-center justify-between px-6 py-5 border-b border-brand-slate-100">
          <div>
            <h3 className="text-lg font-bold text-brand-slate-900">
              Pedido {pedido.codigo_pedido_web}
            </h3>
            <p className="text-xs text-brand-slate-400 mt-0.5">{pedido.fecha} · {pedido.empresa}</p>
          </div>
          <div className="flex items-center gap-3">
            <StatusBadge status={pedido.status} />
            <button
              onClick={onClose}
              className="w-8 h-8 rounded-lg hover:bg-brand-slate-100 flex items-center justify-center transition-colors"
            >
              <svg className="w-4 h-4 text-brand-slate-500" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
        ...
        <div className="overflow-y-auto max-h-[60vh]">
          ...
          {/* Header tabla */}
          <div className="grid grid-cols-12 gap-4 px-6 py-3 bg-brand-slate-50 border-b border-brand-slate-100 text-xs font-semibold text-brand-slate-500 uppercase tracking-wider">
          ...
          {pedido.items?.map((item: ItemPedidoWeb) => (
            <div key={item.id} className="grid grid-cols-12 gap-4 px-6 py-4 border-b border-brand-slate-100 last:border-0 items-center">
              ...
              <div className="col-span-2 text-center text-sm text-brand-slate-700">
                {item.cantidad} <span className="text-brand-slate-400 text-xs">{item.unidad_medida}</span>
              </div>
        ...
        {/* Footer */}
        <div className="px-6 py-5 border-t border-brand-slate-100 space-y-4">

          {/* Observaciones */}
          {pedido.observaciones && (
            <div className="p-4 rounded-xl bg-brand-slate-50">
              <p className="text-xs font-semibold text-brand-slate-500 mb-1">Observaciones</p>
              <p className="text-sm text-brand-slate-700">{pedido.observaciones}</p>
            </div>
          )}
          ...
          <div className="flex items-center justify-between pt-1">
            <span className="text-sm font-semibold text-brand-slate-700">Total</span>
    ...
    <div className="bg-white/70 backdrop-blur-xl rounded-2xl border border-white/20 shadow-xl p-6 flex flex-col sm:flex-row sm:items-center gap-4 hover:-translate-y-0.5 transition-transform duration-200 animate-slide-up">
      ...
      <div className="flex-1 min-w-0 space-y-1">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-mono text-sm font-semibold text-brand-teal-600">
            {pedido.codigo_pedido_web}
          </span>
          <StatusBadge status={pedido.status} />
        </div>
        <p className="text-sm font-semibold text-brand-slate-900 truncate">{pedido.empresa}</p>
        <p className="text-xs text-brand-slate-400">{pedido.fecha}</p>
      </div>

      {/* Total */}
      <div className="text-right shrink-0">
        <p className="text-lg font-bold text-brand-slate-900">
          {pedido.moneda ?? "$"} {pedido.total.toFixed(2)}
        </p>
        <p className="text-xs text-brand-slate-400">Total del pedido</p>
      </div>

      {/* Acción */}
      <button
        onClick={() => onVerDetalle(pedido.id)}
        className="shrink-0 flex items-center gap-2 px-4 py-2 rounded-xl bg-brand-slate-100 hover:bg-brand-teal-50 hover:text-brand-teal-700 text-brand-slate-600 text-sm font-medium transition-all duration-200"
      >
        Ver detalle
    ...
    <div className="space-y-10 animate-fade-in">
      ...
      <div className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div className="flex flex-col gap-1">
          <p className="text-xs font-semibold text-brand-teal-600 uppercase tracking-widest">
            Historial
          </p>
          <h1 className="text-3xl font-bold text-brand-slate-900">Mis Pedidos</h1>
          <p className="text-brand-slate-500 mt-1">Consulta el estado de tus pedidos.</p>
        </div>
            ...
            <button
              ...
              className={`flex items-center gap-2 px-4 py-2 rounded-xl text-sm font-medium transition-all duration-200 ${
                filtroStatus === s
                  ...
                  : "bg-white/70 text-brand-slate-600 hover:bg-brand-teal-50 hover:text-brand-teal-700 border border-white/20 backdrop-blur-xl shadow-sm"
              }`}
            >
              <span className="capitalize">{s === "todos" ? "Todos" : STATUS_CONFIG[s].label}</span>
              <span className={`text-xs px-1.5 py-0.5 rounded-full font-semibold ${
                filtroStatus === s ? "bg-white/20 text-white" : "bg-brand-slate-100 text-brand-slate-500"
              }`}>
                {contadores[s]}
              </span>
            </button>
      ...
      {/* ── Modal detalle ── */}
      {loadingDetalle && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-brand-slate-900/40 backdrop-blur-sm">
          <div className="w-12 h-12 border-4 border-brand-teal-400 border-t-transparent rounded-full animate-spin" />
        </div>
      )}
