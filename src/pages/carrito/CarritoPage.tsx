...
function CarritoItemRow({ item, empresaId }: { item: CarritoItem; empresaId: number }) {
  ...
  return (
    // En mobile (y en tablet, con el sidebar fijo ya ocupando ~256px desde
    // md:) no entran de una sola fila la imagen, el nombre, los controles
    // +/-, el precio y el botón de borrar (menos ancho del que ocupan solo
    // esos elementos de tamaño fijo) — el nombre terminaba invisible,
    // apretado a cero. Por eso pasa a dos filas hasta lg: (recién ahí el
    // contenido tiene sidebar + espacio de sobra), arriba imagen+datos,
    // abajo controles+precio+borrar.
    <div className="flex flex-col lg:flex-row lg:items-center gap-3 lg:gap-5 py-5 border-b border-brand-neutral-100 last:border-0 animate-fade-in">
      ...
      <div className="flex items-center gap-4 lg:gap-5 flex-1 min-w-0">
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-xs text-brand-neutral-500 mt-0.5 flex items-center gap-1.5 flex-wrap">
            {tieneDescuento && (
      ...
      <div className="flex items-center justify-between lg:justify-end gap-4 lg:gap-5 pl-[80px] lg:pl-0">
